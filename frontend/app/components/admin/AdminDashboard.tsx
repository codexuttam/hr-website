...
import {
  ...
  Briefcase, ArrowRight, Sparkles, Code2
} from 'lucide-react';
...
const ADMIN_FEATURES = [
  ...
  },
  {
    ...
    desc: 'Review & moderate community posts',
    active: true
  },
  {
    name: 'Analytics',
  ...
  },
  {
    ...
    desc: 'Create & manage LeetCode-style challenges',
    active: true
  },
  {
    name: 'Student Code Logs',
    href: '/admin/problems?panel=submissions',
    icon: FileText,
    gradient: 'from-indigo-500 to-purple-600',
    bg: 'bg-indigo-50 dark:bg-indigo-900/20',
    text: 'text-indigo-600 dark:text-indigo-400',
    desc: 'Review submitted code, test results, and AI feedback',
    active: true
  },
];
...
const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({ user }) => {
  ...
  async function loadStats() {
    try {
      const [usersRes, quizzesRes, resumesRes, attemptsRes, applicationsRes, problemsRes, submissionsRes] = await Promise.all([
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4">
          {[
            ...
            { label: 'Problems',         value: stats.totalProblems,     icon: Code2,      color: 'text-cyan-600',    bg: 'bg-cyan-50 dark:bg-cyan-900/20'    },
            { label: 'Code Submissions', value: stats.totalSubmissions,  icon: BarChart3,  color: 'text-violet-600',  bg: 'bg-violet-50 dark:bg-violet-900/20'},
          ].map((stat, i) => (
